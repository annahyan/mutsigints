...
for(tissue in names(PCAWG.sig.path.interactions)) {
    ...
    for (metric.name in names(PCAWG.sig.path.interactions[[tissue]])) {
        ...
        for(i in 1:nrow(non.zero.values)) {
            ...
            p.values[sig, path] = min(percentile(PCAWG.sig.path.nulls[[tissue]][[metric.name]][sig, path, ] ,
                                                  metric.values[sig, path]),
                                      1 - percentile(PCAWG.sig.path.nulls[[tissue]][[metric.name]][sig, path, ] ,
                                                  metric.values[sig, path]))
...
pp.PCAWG.combined.summaries = ggdraw() +
  draw_plot(pp.OR + ggtitle("Co-occurrence"), x = 0, y = 0.61, width = 1, height = .4) +
  draw_plot(pp.PTS + ggtitle("lm(signature ~ pathway)"), x = 0, y = .22, width = 1, height = .4) +
  draw_plot(pp.STP + ggtitle("glm(pathway ~ signature)"), x = 0, y = 0, width = 0.42, height = 0.22) 
