...
In total we have `r nrow(mutated.pathways.tissues)` samples and 
`r unique(mutated.pathways.tissues$Cancer.Types) %>% length()` tissues.

```{r}
# [1] 1552
PCAWG.mutated.pathways.occ.plot = mutated.pathways.tissues %>% 
...
```{r}
tissue.odds.ratio.unadjusted = pcawg_sig_pathway_int(sigs.input = PCAWG.full.subset.ann.pathways,
                                 pathways.input = mutated.pathways.tissues,
                                 interaction_function = get_sig_path_assocs, 
                                 path.min.tissues = 30,
                                 p.val.threshold = 0.05,
...
ggsave(plot = pp.odds.ratio.unadjusted,
               filename = file.path(fig.dir, "pathway_to_signature", "by_tissues", "oddsratio",
                                    "PCAWG_summarized_sig_pathway_unadjusted.dotplot.pdf"),
               height = 3, width = 5, bg = "white", device = cairo_pdf, family = "DejaVu Sans")
```
...
ggsave(plot = pp.logged.excl.melanoma,
               filename = file.path(fig.dir, "pathway_to_signature", 
                                    "by_tissues", "lm_log", 
                                    "PCAWG_logged_summarized_OLS_excl_melanoma_path_to_sigs.png"),
               height = 2, width = 2.4, bg = "white")

ggsave(plot = pp.logged.excl.melanoma,
               filename = file.path(fig.dir, "pathway_to_signature", 
                                    "by_tissues", "lm_log", "PCAWG_logged_summarized_OLS_excl_melanoma_path_to_sigs.pdf"),
    ...
    if (any(abs(tissue.log.coefs.mats.log[[tissue]]) > 0) ) {
        ...
        ggsave(plot = int.heatmap,
               filename = file.path(fig.dir, "signature_to_pathway", 
                                    "by_tissues", "log_log", 
                                    paste0("PCAWG_", tissue, "_path_tilde_sig.dotplot.png")),
               height = 5, width = 6, bg = "white")
    }
}
