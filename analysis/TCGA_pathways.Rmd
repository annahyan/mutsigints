...
for(tissue in names(TCGA.sig.path.interactions)) {
    ...
    for (metric.name in names(TCGA.sig.path.interactions[[tissue]])) {
        ...
        for(i in 1:nrow(non.zero.values)) {
            ...
            p.values[sig, path] = min(percentile(TCGA.sig.path.nulls[[tissue]][[metric.name]][sig, path, ] ,
                                                  metric.values[sig, path]),
                                      1 - percentile(TCGA.sig.path.nulls[[tissue]][[metric.name]][sig, path, ] ,
                                                  metric.values[sig, path]))
        ...
        p.values =  ## replace NA's with 1 in upper.tri
            pmin(1, p.values, na.rm = TRUE)
