    ...
    for (tissue in names(null.list)) {
        ...
        quantiles.mat = do.call(rbind, 
                                lapply(null.list[[tissue]], function(x) {
                                    x = x[rowSums(is.na(x)) != ncol(x), colSums(is.na(x)) != nrow(x), drop = FALSE]
                                    if(length(x) == 0) {
...
```{r, warning = FALSE}
tissue.odds.ratio = sig_pathway_int(sigs.input = TCGA.full.subset.ann.pathways,
...
# tissue.odds.ratios.null = sig_pathway_int_null(sigs.input = TCGA.full.subset.ann.pathways,
#                                  pathways.input = TCGA.pathways.tissues,
#                                  interaction_function = get_sig_path_assocs,
#                                  path.min.tissues = 30,
#                                  p.val.threshold = 0.05,
...
#                                  N = 1000)
# 
# saveRDS(tissue.odds.ratios.null, file = file.path(out.dir, "TCGA.sig.path.OR.nulls.RDS"))
tissue.odds.ratios.null = readRDS(file = file.path(out.dir, "TCGA.sig.path.OR.nulls.RDS"))
```

```{r}
quant = 0.95
tissue.OR.quantiles = get_quants_from_null(tissue.odds.ratios.null, 
                                           TCGA.full.subset.ann.pathways,
                                           TCGA.pathways.tissues, 
                                           quant)
```
...
pp.odds.ratio.null.adjusted = plot_all_counts(tissue.odds.ratio.null.adjusted, 
                                              ...
                                              expand.mult = c(0.08, 0.08))

ggsave(plot = pp.odds.ratio.null.adjusted,
               filename = file.path(fig.dir, "pathway_to_signature", "by_tissues", "oddsratio",
                                    "TCGA_summarized_sig_pathway_null_adjusted.dotplot.pdf"),
               height = 3, width = 4.2, bg = "white", device = cairo_pdf, family = "DejaVu Sans")
```
...
```{r}
sig.path.oddsratio.bipartite = minor_plot(plot_bipartite2(tissue.odds.ratio.unadjusted),
...
ggsave(plot = sig.path.oddsratio.bipartite,
       ...
       height = 5, width = 6, bg = "white", device = cairo_pdf, family = "DejaVu Sans")
```
...
```{r, echo = FALSE, include=FALSE}
# tissue.lm.ratios.null = sig_pathway_int_null(sigs.input = TCGA.full.subset.ann.pathways,
#                                  pathways.input = TCGA.pathways.tissues,
#                                  interaction_function = get_sig_path_lms,
#                                  robust = FALSE,
#                                  path.min.tissues = 30,
...
# saveRDS(tissue.lm.ratios.null, file = file.path(out.dir, "TCGA.sig.path.lm.nulls.RDS"))
tissue.lm.ratios.null = readRDS(file = file.path(out.dir, "TCGA.sig.path.lm.nulls.RDS"))
```
    ...
    if (any(abs(tissue.lm.null.adjusted[[tissue]]) > 0) ) {
        ...
        ggsave(plot = int.heatmap,
               filename = file.path(fig.dir, "pathway_to_signature",
                                    "by_tissues", "lm_log",
                                    paste0("TCGA_", tissue, "_sig_tilde_pathway.lm.null.adjusted.dotplot.png")),
               height = 5, width = 6, bg = "white")
    }
}
```
...
### Logistic regression getting pathways from signatures across tissues
...
tissue.glm.ratios.null = sig_pathway_int_null(sigs.input = TCGA.full.subset.ann.pathways,
                                 pathways.input = TCGA.pathways.tissues,
                                 interaction_function = get_sig_path_lms,
                                 path.min.tissues = 30,
                                 p.val.threshold = 0.05,
...
saveRDS(tissue.glm.ratios.null, file = file.path(out.dir, "TCGA.sig.path.glm.nulls.RDS"))
# tissue.glm.ratios.null = readRDS(file = file.path(out.dir, "TCGA.sig.path.glm.nulls.RDS"))
```
...
tissue.glm.null.adjusted = setNames(
    lapply(names(tissue.log.coefs.mats.log),
           function(tissue) {
               ...
               colnames(observed) = sub("-", ".", sub(" ", ".", colnames(observed) ) )
               nm = tissue.glm.quantiles[[tissue]]
               nm = observed[rownames(observed), colnames(observed)]
...
```{r}
pp.log.log = plot_all_counts(tissue.glm.null.adjusted, threshold = 0.1, psize = 6,
...
ggsave(plot = pp.log.log,
               ...
               height = 2.8, width = 4, bg = "white", device = cairo_pdf, family = "DejaVu Sans")
```
...
```{r}
pp.TCGA.combined.summaries = ggarrange(pp.odds.ratio.null.adjusted + ggtitle("Co-occurrence"), 
