...
get_tissue_null_metrics_dist = function( tissue, dataset, metric_functions_list, 
                                      N = 1000) {
...
```{r, warning = FALSE}
PCAWG.tissue.null.quantiles.95 = list()

PCAWG.out.dir = here("data", "RDS", "PCAWG", "signatures") 
for (tissue in names(PCAWG.tissue.counts)) {
    
    cat(tissue, "\n")
    pcawg.quants = get_tissue_null_quantiles(
        tissue, PCAWG.full.subset.ann, 
        metric_functions, 
        quant = 0.95)
    PCAWG.tissue.null.quantiles.95[[tissue]] = pcawg.quants
        
}
saveRDS(PCAWG.tissue.null.quantiles.95, file = file.path(PCAWG.out.dir, "PCAWG.null.model.95.quants.RDS"))
```
...
```{r, warning = FALSE}
TCGA.tissue.null.quantiles.95 = list()

TCGA.out.dir = here("data", "RDS", "TCGA", "signatures") 
for (tissue in names(TCGA.tissue.counts) ) {
    
    cat(tissue, "\n")
    tissue.quants = get_tissue_null_quantiles(tissue, TCGA.full.subset.ann, metric_functions)
    TCGA.tissue.null.quantiles.95[[tissue]] = tissue.quants
        
}
saveRDS(TCGA.tissue.null.quantiles.95, file = file.path(TCGA.out.dir, "TCGA.null.model.95.quants.RDS"))
```
