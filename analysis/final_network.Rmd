...
plot.mixed.layout = function(graph.input, 
    ...
    pp = graph.input %>% ggraph(x = node.coordinates$x, y = node.coordinates$y) + 
        ...
        scale_edge_width(breaks = c(1,2,4,13), range = c(0.5,3)) +
...
sig.sig.all.network = plot.mixed.layout(pcawg.sig.sig.net, circular.node.order = node.order)
ggsave(plot = minor_plot(sig.sig.all.network, expand.factor = 0.05), 
       filename = file.path(fig.dir, "sig.sig.all.network.pdf"),
