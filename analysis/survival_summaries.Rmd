...
p = plot_param_piechart(pcawg.sig.sig.comb, param = "log(total_muts + 1)", add.param = TRUE)
...
<!-- pcawg.TMB.explore = pcawg.sig.sig.comb %>% -->
<!--     group_by(cond) %>% -->
<!--     summarise(non.param = all(params != "log(total_muts + 1)")) %>% -->
<!--     filter(non.param) %>% -->
<!--     ungroup() %>% -->
<!--     do(add_param_row(., pcawg.sig.sig.comb, "log(total_muts + 1)")) %>% -->
<!--     bind_rows(pcawg.sig.sig.comb, .) %>% -->
<!--     arrange(cond, params) -->
...
```{r}
p = plot_param_piechart(pcawg.sig.sig.comb, param = "age_at_diagnosis", add.param = TRUE)
...
```{r}
pcawg.age.ggforest = pcawg.sig.sig.comb %>% surv_add_param_fillers (., param = "age_at_diagnosis") %>% 
